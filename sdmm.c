...
)
{
	...
	{
		uint32_t ui32RcvDat;
		SSIDataPut(SSI2_BASE, *buff++); /* Write the data to the tx fifo */
		SSIDataGet(SSI2_BASE, &ui32RcvDat); /* flush data read during the write */
	}
...
)
{
	while(bc--)
	{
		...
		SSIDataPut(SSI2_BASE, 0xFF); /* write dummy data */

		SSIDataGet(SSI2_BASE, &ui32RcvDat); /* read data frm rx fifo */

		*buff++ = (BYTE)ui32RcvDat;
	}
}
...
)
{
	...
	/*
	 * Setup SSI2 Port (Platform dependent)
	 * CS:  E5
	 * RX:  B6
	 * TX:  B7
	 * CLK: B4
	...
	//set pins to ssi mode (enable pullups and so on)
	GPIOPinTypeSSI(GPIO_PORTB_BASE, GPIO_PIN_4 | GPIO_PIN_6 | GPIO_PIN_7);

	GPIOPadConfigSet(GPIO_PORTB_BASE, GPIO_PIN_6, GPIO_STRENGTH_4MA,
			GPIO_PIN_TYPE_STD_WPU);
	GPIOPadConfigSet(GPIO_PORTB_BASE, GPIO_PIN_4 | GPIO_PIN_7,
